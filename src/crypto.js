...
function deriveKey(orgId) {
  ...
  if (!secret) {
    // If no SESSION_SECRET, we can't encrypt. This matches the auth.js warning behavior.
    return null;
  }
 ...
 * @param {string} plaintext - The OAuth refresh token (or any secret)
 * @param {string} orgId - Organization UUID (used to derive encryption key)
 * @returns {string} Encrypted string in format "enc:v1:<iv>:<ciphertext>:<tag>"
 */
export function encryptToken(plaintext, orgId) {
  ...
  if (!key) {
    // No SESSION_SECRET — store plaintext (with warning logged once at startup)
    return plaintext;
  }
 ...
 * Handles both encrypted ("enc:v1:...") and legacy plaintext tokens.
 * @param {string} stored - The stored token value
 * @param {string} orgId - Organization UUID (used to derive decryption key)
 * @returns {string} Plaintext token
 */
export function decryptToken(stored, orgId) {
  if (!stored) return null;

  // If not encrypted (legacy plaintext token), return as-is
  ...
  const key = deriveKey(orgId);
  if (!key) {
    // Can't decrypt without SESSION_SECRET
    throw new Error('Cannot decrypt token: SESSION_SECRET not set');
  }
...
/**
 * Re-encrypt a token (for migration from plaintext to encrypted).
 * If already encrypted, returns as-is. If plaintext, encrypts it.
 * @param {string} stored - The stored token value
 * @param {string} orgId - Organization UUID
...
export function ensureEncrypted(stored, orgId) {
  if (!stored) return null;
  if (isEncrypted(stored)) return stored; // Already encrypted
